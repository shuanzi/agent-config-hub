...
export class HarnessPeakRssSampler {
  ...
  async capture() {
    ...
    const lifecycle = lifecycleFrom(this.lifecyclePath);
    ...
    if (
      ...
      this.failure = inconclusive('harness lifecycle identity invalid');
      return;
    }
    if (this.harnessPid !== null && this.harnessPid !== lifecycle.pid) {
      this.failure = inconclusive('harness PID changed');
      return;
    }
    this.harnessPid = lifecycle.pid;
    if (lifecycle.normalExit === true) {
      // 终态一经 attested 就不再查询已退出 PID；仅保留此前成功窗口内样本。
      if (this.timer !== null) clearInterval(this.timer);
      this.timer = null;
      return;
    }
    if (lifecycle.normalExit !== false) {
      this.failure = inconclusive('harness normal exit state invalid');
      return;
    }
    try {
      this.samples.push(processTreeRssBytes(await this.readRows(), lifecycle.pid));
    } catch (error) {
      this.failure = error instanceof Error ? error : inconclusive('RSS capture failed');
    }
  }
  ...
  async finalize({ waitForLifecycleMs = 5000 } = {}) {
    ...
    return {
      ...
      normalExit: lifecycle?.normalExit === true,
    };
  }
}
