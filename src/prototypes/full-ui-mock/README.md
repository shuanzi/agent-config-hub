...
- 宽／中窗口固定为“配置上下文 → 资产类型 → 唯一主表面”三列；第一列只展示全局配置、项目配置标题与项目列表，第二列在 selected 候选中只展示 Skills、长期指令、Subagents。shared `AssetType` 与 legacy A/B/C 仍保留 Hooks 作为历史证据。
- 主表面在列表、结构化详情、源码编辑及事务旅程之间替换，不常驻第四检查器。
...
- 窄窗口使用“上下文 → 类型 → 列表 → 详情／旅程”单表面栈，并保留返回路径、dirty guard、Esc、焦点恢复和减少动态效果。
