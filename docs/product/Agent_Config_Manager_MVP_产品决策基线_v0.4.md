...
Skills 保留仓库发现、安装、导入、更新、卸载、备份恢复及四 Agent 即时投影语义。每条记录携带完整 ownership target；在“全部”中发起没有现有记录可派生 target 的操作，必须先明确选择 global 或具体 project。
...
Subagents 保留仓库发现、安装、更新、卸载、备份恢复及 per-Agent 即时投影语义。完整 ownership target 与“全部”中的显式 target 规则和 Skills 相同。项目级 Codex Subagent 路径未获支持，因此对应安装和 toggle 入口禁用并说明原因。

## 3. UI 与响应式基线

宽屏采用 selected B2 的 `资产类型 rail → 配置上下文 rail → 主工作区`。第一栏固定为 Skills、长期指令、Subagents，设置位于底部；第二栏固定为“全部／全局配置／真实项目列表”。当前 Agent 保留在 Header 的紧凑控件中，不占用第二栏，也不影响长期指令的两种固定文档。
