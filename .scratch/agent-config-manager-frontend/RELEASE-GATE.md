...
**Direct blockers:** FE-01 至 FE-10 全部为 `done` 并附完成证据。前端契约与整机技术方案已于 2026-07-27 验收，`ARCH-GATE` 已关闭。

## 目的与边界

本门禁只汇总已完成的实现证据，不新增前端行为票据、不接管 FX-01 至 FX-18 的主归属，也不在契约未验收前启动。它承接跨票据全回归、真实 adapter 契约回归、构建、打包及负向范围检查；FX-12 的单一集成 UI 旅程仍只属于 FE-10。

## 验证命令契约

**状态：** `planned / unverified`。当前没有 `package.json`、可运行 app、签名产物或实际命令证据。

**统一入口：** `npm run verify:release`

**前置条件：**

- FE-01 至 FE-10 均为 `done` 且各自 `verify:ticket` manifest 可读取；`ARCH-GATE` 已于 2026-07-27 关闭；
...
2. L1：全部 Rust、Vitest、wire、adapter bundle、事务与安全负向回归；
3. L2：FX-01 至 FX-18 的 mock renderer 全回归；只取得 renderer/mock provenance；
4. L3：专用 Tauri test harness 对同一个 `FrontendGatewayContract` 做真实 adapter 全回归，并覆盖隔离 command/event、prepare/apply、索引、更新与恢复 tracer；
...
- [ ] 所有 FX-01 至 FX-18 都由各自主票据完成并保留聚焦旅程证据；
