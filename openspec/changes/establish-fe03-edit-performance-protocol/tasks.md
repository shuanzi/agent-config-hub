...
- [ ] 1.1 对本 change 的 proposal、delta spec、design 和 tasks 执行独立只读审查，逐项修复有效 finding，并记录审查不构成 FE-03 功能、PF 或 closure 证据。
...
- [ ] 2.1 在原 change 的 3.19 gate 通过后，为 3.20 所需三类 `editAsset` 草稿、长期指令首次实际变更建草稿、dirty guard 和未知内容保真/只读先新增可失败的 L0/L1 contract/domain/wire 测试，再以最小实现使其转绿；不纳入 apply 或 L3。
- [ ] 2.2 为原 change 的 3.21 生成或复验 TypeScript wire、vectors 与 drift，并以 RED→GREEN 实现共享草稿状态和类型特定编辑表面；验证生成物和 vector 不含敏感明文、grant 或真实路径。
- [ ] 2.3 为原 change 的 3.22 运行仅限 FE-03 的 L0/L1 草稿、保真和 dirty-guard focused checks，以及 mock `FrontendGateway` 的 L2 编辑 journey；记录 `functional checks complete` 为 task-only、`non-closure` 结果，并明确无 L3、actual Tauri IPC、磁盘写入、真实授权或真实 write credit。
- [ ] 2.4 在 2.1–2.3 全部完成前，不实施、采集或运行 `fe03-edit-pf/v1` 的 descriptor、collector、measurement、comparison 或 formal closure；不得以任何功能检查点勾选 FE-03、更新 frontier 或替代后续 gate。

## 3. Edit-PF protocol 与 grant 消费实现 gate

- [ ] 3.1 仅在 2.3 已记录 `functional checks complete` 后，创建 `fe03-edit-pf/v1` 的 `PF-02-edit-v1` 与 `PF-03-edit-v1` descriptor 及各自物理 path，定义 edit 输入、草稿投影、文件切换、profile/seed 和 fail-closed identity 校验；不得复用、改名或修改任何 read descriptor。
...
- [ ] 3.4 在实现 grant consumer 前复核：Rust-first opaque authoritative DTO 可由既有 `SensitiveRevealQuery`/`FrontendGateway.read` 和 read/session seam 消费，且无新 command、trust boundary 或第二 serialization source；若任一前提不成立，标记 `ARCH-GATE: reopen-required`，停止 FE-03 恢复并回主任务请求用户架构决定。
- [ ] 3.5 仅在 3.4 通过后，实现 Rust 权威 DTO 的 mock/session 消费及 L0/L1/L2 失效、重新遮蔽测试：绑定 asset/file/segment/revision/`scope=modify`/surface/TTL，切换或过期即失效；mock 不得签发授权或取得 actual authorization、IPC、磁盘写入、L3 或真实 write credit，grant/明文不得进入日志、缓存、可序列化草稿、fixture、vector 或 PF artifact。

## 4. Additive verification route gate

- [ ] 4.1 仅在 3.1–3.5 通过后，为 FE-03 增加 edit-only registry entry 和 `verify:ticket` route，将 descriptor bytes/digest、fixture digest、collector/config/runner/evaluator/attestation module identity 与实际 bytes digest、actual L2 SUT module graph digest/attestation、measurement-input graph、raw samples、manifest、budget lineage 与 ticket `FE-03` 逐项绑定并 fail closed；不得改变 legacy FE-02 read route。
...
- [ ] 5.2 将两条 baseline 均明确标记为 `baseline-collected`、`budget-not-frozen` 和 `non-closure`，并验证此状态不继承 FE-02 budget、不触发 formal comparison/closure、不勾选 FE-03 且不推进 frontier。
...
- [ ] 7.2 仅在 7.1 成功后运行 edit-only formal comparison；任何 budget、descriptor、fixture、attestation、manifest 或 comparison 的 fail/inconclusive 均阻断后续 closure。
...
- [ ] 8.3 对本 change 的 Markdown 运行目标 Prettier，对 tracked/untracked changed paths 运行 whitespace 检查，并审计精确 changed paths 仅属于获准的 future FE-03 implementation/protocol scope；不得通过本 validation 修改既有 read PF、budget、waiver、collector/verifier、历史 evidence 或其他 change。
