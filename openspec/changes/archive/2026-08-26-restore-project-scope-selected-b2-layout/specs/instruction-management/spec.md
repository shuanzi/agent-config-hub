...
**Reason**: 长期指令已收敛为每个 target 的 `CLAUDE.md` 与 `AGENTS.md` 两种固定 live 文档，不再暴露预设集合。

**Migration**: 既有数据库行仅作为未暴露的 legacy 数据保留；用户改为直接编辑对应固定文档。

### Requirement: 互斥激活与 live 写入

**Reason**: 新模型不维护 per-Agent enable；文档的适用 Agent 由文件种类固定决定。

**Migration**: 既有 enabled preset 不参与读取、保存或 override 搬迁；当前 live 文件继续按固定文档规则管理。

### Requirement: 从 live 文件导入与查看

**Reason**: 固定文档列表直接读取 live 文件，已不需要导入为预设或单独的预设对比流程。

**Migration**: 用户在固定文档详情中直接查看和保存当前 live 内容。
